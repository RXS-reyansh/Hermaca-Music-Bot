    ...
    prefixExecute: async (message, args, client) => {
        ...
        if (args.length) {
            ...
            if (firstArg === 'bot') {
                ...
                if (!targetGuild.icon) return messages.error(message.channel, 'This server does not have an icon.');
                const avatarURL = targetGuild.iconURL({ dynamic: true, size: 4096 });
                const embed = buildImageEmbed(`${emojis.blackbutterfly} ${targetGuild.name}'s icon`, avatarURL, message.author);
