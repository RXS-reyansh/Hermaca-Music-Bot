...
function printServerList(serversData) {
    ...
    for (const guild of guilds.values()) {
        const serverInfo = serversData[guild.id] || {
            name: guild.name,
            inviteCode: 'N/A'
        };
...
client.on(Events.InteractionCreate, async interaction => {
    ...
    if (interaction.type === InteractionType.ApplicationCommandAutocomplete) {
        ...
        if (interaction.commandName === 'help') {
            ...
            const filtered = commandNames.filter(name =>
                    name.toLowerCase().includes(focusedValue.toLowerCase())).slice(0, 25);
    ...
    try {
        ...
        case 'lyrics': {
                ...
                if (lyricsData.clientName && !lyricsData.cached) {
                    logMessage += ` | API Client: ${lyricsData.clientName}`;
                }

                log('LYRICS', logMessage);
        ...
        case 'nowplaying': {
                ...
                embed.addFields([{
                            name: 'Artist',
                            value: `${emojis.blackbutterfly} ${player.current.info.author || "Unknown"}`,
                            inline: true
                        }, {
                            name: 'Duration',
                            value: `${emojis.blackbutterfly} ${formatDuration(player.current.info.length)}`,
                            inline: true
                        }, {
                            name: 'Requested By',
                            value: `${emojis.blackbutterfly} ${(player.current.info.requester && player.current.info.requester.tag) || "Unknown"}`,
                            inline: true
                        }
                    ])
                .setFooter({
...
`${emojis.music} Available Filters: nightcore, vaporwave, 8d, 16d, chipmunk, deepbass, reset`
                        ...
                        .setFooter({
                            text: "Use /filter reset to clear filters"
                        });
        ...
        case 'status': {
                const player = client.riffy.players.get(guild.id);
                if (!player)
                    return await interaction.editReply(`${emojis.error} | No active player found!`);
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields([{
                                name: 'Status',
                                value: player.playing ? `${emojis.playboy} Playing` : `${emojis.pause} Paused`,
                                inline: true
                            }, {
                                name: 'Volume',
                                value: `${emojis.playboy} ${player.volume}%`,
                                inline: true
                            }, {
                                name: 'Loop Mode',
                                value: `${emojis.playboy} ${player.loop === "queue" ? 'Queue' : 'Disabled'}`,
                                inline: true
                            }
                        ]);
                break;
        ...
        case 'ping': {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setFooter({
                        text: `Database on MongoDB • Powered by ${client.hostingService || 'Unknown'}`
                    })
                    .setTimestamp();
...
`**Usage:** \`${usage}\``
                    ...
                    const embed = new EmbedBuilder()
                        ...
                        .setFooter({
                            text: `Use ${client.prefix}help for all commands`
                        });
        ...
        case 'song-quote': {
                ...
                try {
                    ...
                    const {
                        createSongQuoteImage
                    } = require("./utils/songQuoteGenerator");
                    ...
                    collector.on('collect', async i => {
                        try {
                            ...
                            await i.reply({
                                content: `${emojis.success} Image sent to your DMs!`,
                                flags: MessageFlags.Ephemeral
                            });
                        } catch (error) {
                            await i.reply({
        ...
        case 'resetmystats': {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setFooter({
                        text: 'You have 30 seconds to decide'
                    });
        ...
        case 'setbio': {
                ...
                const {
                    result: finalBio,
                    invalid
                } = await replaceEmojiPlaceholders(withNewlines, client, guild);
                ...
                if (match) {
                    ...
                    try {
                        const {
                            ...
                            getPlaylistDetails
                        } = require("./utils/spotifyPlaylists.js");
                            ...
                            for (let i = 0; i < fullTracks.length; i += CONCURRENCY_LIMIT) {
                                ...
                                resolvedTracks.forEach((t, idx) => {
                                    ...
                                    if (original && original.thumbnail) {
                                        if (!player._spotifyThumbs)
                                            player._spotifyThumbs = new Map();
...
async function handleLyrics(context, isInteraction = false) {
    ...
    if (isInteraction) {
        ...
        loadingMsg = await context.channel.send(`${emojis.loading} | Searching for lyrics...`);
    }
    ...
    let provider = lyricsData.cached ? `${lyricsData.source}/Cache` : lyricsData.source;
    let logMessage = `${trackArtist} - ${trackTitle} | Provider: ${provider} | Requested by: ${requester.tag} | Server: ${guildName}`;
    ...
    if (isInteraction) {
        await sendLyricsEmbeds(context, true, null, lyricsData.lyrics, lyricsData.source, trackArtist, trackTitle, requester);
    } else {
        if (loadingMsg) {
            await loadingMsg.delete().catch(() => {});
        }
        await messages.lyrics(context.channel, trackTitle, lyricsData, trackArtist, requester);
    }
}
async function sendLyricsEmbeds(context, isInteraction, loadingMsg, lyrics, source, trackArtist, trackTitle, requester) {
...
async function handleNowPlaying(context, isInteraction = false) {
    ...
    if (isInteraction) {
        ...
        embed.addFields([{
                    name: 'Artist',
                    value: `${emojis.blackbutterfly} ${currentTrack.info.author || "Unknown"}`,
                    inline: true
                }, {
                    name: 'Duration',
                    value: `${emojis.blackbutterfly} ${formatDuration(currentTrack.info.length)}`,
                    inline: true
                }, {
                    name: 'Requested By',
                    value: `${emojis.blackbutterfly} ${(currentTrack.info.requester && currentTrack.info.requester.tag) || "Unknown"}`,
                    inline: true
                }
            ])
        .setFooter({
...
async function canBotUseSticker(sticker) {
    ...
    if (sticker.format && sticker.format > 3)
        return false;
...
async function handleAfkSlash(interaction, reason, imageAttachment) {
    ...
    const {
        result: finalReason,
        invalid
    } = await replaceEmojiPlaceholders(withNewlines, interaction.client, guild);
    if (invalid.length > 0) {
    ...
    collector.on('collect', async i => {
        if (i.customId === 'confirm_afk') {
            ...
            if (success) {
                await i.update({
                    embeds: [new EmbedBuilder().setColor(config.embedColor).setDescription(`${emojis.success} AFK set in all servers!`)],
                    components: []
                });
            } else {
                await i.update({
                    embeds: [new EmbedBuilder().setColor(0xff5555).setDescription(`${emojis.error} Failed to set AFK status.`)],
                    components: []
                });
            }
        } else {
            await i.update({
                embeds: [new EmbedBuilder().setColor(0xff5555).setDescription(`${emojis.error} AFK command cancelled.`)],
                components: []
            });
        }
        collector.stop();
...
async function handleAvatarSlash(interaction, targetUser) {
    ...
    if (hasServerAvatar) {
        const promptEmbed = new EmbedBuilder()
            ...
            .setFooter({
                text: 'You have 60 seconds to decide'
            });
...
async function handleSaySlash(interaction, text, attachment) {
    ...
    const {
        result: finalText,
        invalid
    } = await replaceEmojiPlaceholders(withNewlines, interaction.client, interaction.guild);
    if (invalid.length > 0) {
        return await interaction.editReply(`${emojis.error} | Invalid emoji identifiers:\n${invalid.map(id => `• ${id}`).join('\n')}`);
    }
...
async function handleQuote(context, isInteraction = false, initialOptions = {}) {
    try {
        ...
        if (customText)
            currentOptions.customText = customText;

        const loadingMsg = isInteraction
             ? await context.editReply({
                content: `${emojis.loading} | Generating quote image...`
            })
        ...
        if (isInteraction) {
            ...
            await loadingMsg.delete().catch(() => {});
            sentMsg = await context.channel.send({
                embeds: [embed],
                files: [attachment]
            });
        }
        ...
        collector.on('collect', async i => {
            ...
            if (currentOptions.cancelled) {
                await i.reply({
                    content: 'This quote has been cancelled.',
                    flags: MessageFlags.Ephemeral
                });
                return;
            }
            ...
            if (i.customId === `quote_dm_${user.id}`) {
                try {
                    ...
                    await i.reply({
                        content: `${emojis.error} Could not send DM.`,
                        flags: MessageFlags.Ephemeral
                    });
                }
                return;
            ...
            if (currentOptions.confirmed) {
                await i.reply({
                    content: 'Quote already confirmed. You can only download or send to DM.',
                    flags: MessageFlags.Ephemeral
                });
                return;
            }
...
async function handleHelp(context, isInteraction = false) {
    ...
    const commands = [{
            ...
            name: 'lyrics',
            description: 'Show the lyrics of the current track'
        }, {
            name: 'queue',
            description: 'Show the current queue'
        }, {
            ...
            name: 'servervolume <0-100>',
            description: 'Set permanent volume for this server'
        }, {
            name: 'filter <type>',
            description: 'Add different filters to playback'
        }, {
            name: 'shuffle',
            description: 'Shuffle the current queue'
        }, {
            ...
            name: 'leaderboard',
            description: 'Global ranking of top listeners'
        }, {
            name: 'resetmystats',
            description: 'Reset your personal statistics'
        }, {
            name: 'setspotify <username>',
            description: 'Set your Spotify username'
        }, {
            name: 'playspotify',
            description: 'Play your saved Spotify playlists'
        }, {
...
`${emojis.music} \`${cmd.name}\` - ${cmd.description}`).join('\n'))
            .setImage("https://i.ibb.co/gLM9bMf9/standard.gif")
            .setFooter({
                text: 'Prefix: ~ • Example: ~play <song name>'
            });
...
async function handlePrefixCommand(message, cmd, args) {
    ...
    case 'p': {
            ...
            if (!query)
                return message.reply(`${emojis.error} | Please provide a song to play!`);
    ...
    case 'vol': {
            ...
            if (isNaN(volume))
                return message.reply(`${emojis.error} | Please provide a valid volume level (0-100)!`);
    ...
    case 'add': {
            ...
            if (!song)
                return message.reply(`${emojis.error} | Please provide a song!`);
...
`**Usage:** \`${usage}\``
    ...
    case 'setspotify': {
            ...
            if (!username)
                return message.reply(`${emojis.error} | Please provide your Spotify username!`);
    ...
    case 'av': {
            ...
            if (args.length > 0) {
                ...
                if (firstArg === 'bot') {
                    ...
                    const avatarURL = targetGuild.iconURL({
                        dynamic: true,
                        size: 4096
                    });
...
`${emojis.blackbutterfly} ${targetGuild.name}'s icon`,
                ...
                targetUser = await client.users.fetch(message.author.id, {
                    force: true
                });
                member = await message.guild.members.fetch({
                    user: targetUser.id,
    ...
    case 'bn': {
            ...
            if (args.length > 0) {
                ...
                if (firstArg === 'bot') {
                    ...
                    const bannerURL = targetGuild.bannerURL({
                        dynamic: true,
                        size: 4096
                    });
                    const embed = buildImageEmbed(
`${emojis.blackbutterfly} ${targetGuild.name}'s banner`,
                    ...
                    try {
                        ...
                        if (interaction.customId.startsWith(`steal_emoji_`)) {
                            ...
                            try {
                                ...
                                await stealMessage.edit({
                                    embeds: [successEmbed],
                                    components: []
                                });
                                await interaction.editReply({
                                    content: `${emoji} | Successfully stole as emoji!`
                                });
                                ...
                                if (emojiError.code === 30008)
                                    errorMessage = 'Server has reached maximum emoji limit';
                                else if (emojiError.code === 50045 || emojiError.message.includes('Invalid Form Body'))
                                    errorMessage = 'Invalid image format for emoji';
                                else if (emojiError.message.includes('animated'))
                                    errorMessage = 'Server needs higher boost level for animated emojis';
                                await interaction.editReply({
                            ...
                            try {
                                ...
                                const sticker = await interaction.guild.stickers.create({
                                    ...
                                    if (err.code === 30039)
                                        throw new Error('Server has reached maximum sticker limit');
                                    throw err;
                                });
                                ...
                                await interaction.editReply({
                                    content: `${emojis.success} | Successfully stole as ${isAnimated ? 'animated ' : ''}sticker!`
                                });

                            } catch (stickerError) {
                                console.error('Sticker creation error:', stickerError);
                                let errorMessage = 'Failed to create sticker';
                                if (stickerError.code === 30039)
                                    errorMessage = 'Server has reached maximum sticker limit';
                                else if (stickerError.message.includes('Invalid Form Body'))
                                    errorMessage = 'Invalid image format for sticker (try a static image)';
                                await interaction.editReply({
                                    content: `${emojis.error} | ${errorMessage}`
                                });
                            }
                        ...
                        await interaction.editReply({
                            content: `${emojis.error} | Failed to process asset: ${error.message}`
                        });
                    }
                });
    ...
    case 'song-quote': {
            ...
            if (!text)
                return message.reply(`${emojis.error} | Please provide text for the quote!`);
    ...
    case 'setbio': {
            ...
            const {
                result: finalBio,
                invalid
            } = await replaceEmojiPlaceholders(withNewlines, client, message.guild);
...
`• Division must yield integer result`)
                    .setFooter({
                        text: 'Use these in the counting channel!'
                    })
                    .setTimestamp();
...
`\`${client.prefix}count calc\` – Show calculations guide`)
                    .setFooter({
                        text: 'Use these commands in any channel.'
                    })
                    .setTimestamp();
                return message.channel.send({
                    embeds: [embed]
            ...
            if (subCmd === 'enable') {
                ...
                if (!channelArg)
                    return messages.error(message.channel, 'Please specify a channel (mention or ID).');
...
`${emojis.music} Status: ${resetStatus}`)
                        .setFooter({
                            text: 'Use ~count help for more commands'
                        })
                        .setTimestamp();
                ...
                const config = await client.db.getCountingConfig(message.guild.id);
                if (!config)
                    return messages.error(message.channel, 'Counting is not enabled in this server.');
...
const sendImageWithDMButton = async(channel, embed, requester) => {
    ...
    collector.on('collect', async i => {
        try {
            await i.user.send({
                embeds: [embed.setFooter(null).setTimestamp()]
            });
            await i.reply({
                content: `${emojis.success} Image sent to your DMs!`,
                flags: MessageFlags.Ephemeral
