...
`[Support Server](https://discord.gg/nVfAGH9G67)`,
    ...
    const embed = new EmbedBuilder()
        ...
        .setFooter({
            text: `Select a category from the dropdown below!`
        });
...
function buildCategoryEmbed(guild, categoryKey, categoryName, commands, user, prefix) {
    const embed = new EmbedBuilder()
        ...
        .setImage("https://i.ibb.co/gLM9bMf9/standard.gif")
        .setFooter({
            text: `Use ${prefix}help <command-name> to know more about the command!`
        });
    return embed;
}
...
`${emojis.music} Available Filters: nightcore, vaporwave, 8d, 16d, chipmunk, deepbass`
	...
	lyrics: async (channel, trackTitle, lyricsData, trackArtist, requester) => {
		try {
			if (!lyricsData?.lyrics) {
				return module.exports.error(channel, "Lyrics not found!");
			}
			await module.exports.sendLyricsEmbeds(
				channel,
				lyricsData.lyrics,
				lyricsData.source || 'Genius.com',
				trackArtist,
				trackTitle,
				requester
			);
    ...
    addedToQueue: (channel, track, position) => {
        ...
        embed.addFields([{
                    name: 'Artist',
                    value: `${emojis.hearts} ${track.info.author}`,
                    inline: true
                }, {
                    name: 'Duration',
                    value: `${emojis.hearts} ${getDurationString(track)}`,
                    inline: true
                }, {
                    name: 'Position',
                    value: `${emojis.hearts} #${position}`,
                    inline: true
                }
            ]);
        return channel.send({
    ...
    ping: async function (channel, client, context, hostingService) {
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setFooter({
                    text: `Database on MongoDB • ${hostingText}`
                })
                .setTimestamp();
        ...
        collector.on('collect', async(i) => {
            ...
            if (i.isStringSelectMenu()) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            const embed = new EmbedBuilder()
                                ...
                                .setFooter({
                                    text: 'Playlist loaded (fallback mode)'
                                });
                            if (thumbnail)
                        ...
                        if (result.loadType === 'playlist' && result.tracks?.length > 0) {
                            ...
                            const embed = new EmbedBuilder()
                                ...
                                .setFooter({
                                    text: 'Loaded via Spotify plugin'
                                });
                            if (thumbnail)
                                embed.setThumbnail(thumbnail);
                ...
                const {
                    embed,
                    components
                } = createPageComponents(currentPage);
                try {
                    await i.editReply({
                        embeds: [embed],
                        components: components
